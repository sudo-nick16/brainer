...
const EditAdventureModal = ({ }) => {
    ...
    const editAdventure = () => {
        ...
        let worldSlug = slugify(currentWorld);
        updateState((curr) => {
    ...
    return (
        ...
        >
            <div className='bg-black py-10 px-10 gap-3 flex flex-col w-full relative rounded-3xl border border-accent min-w-[350px] '>
                ...
                <div className='flex items-center gap-x-3 justify-start'>
                    <Button className={`${state.urgent ? '!bg-accent' : ''} !px-6 !py-2 flex-1`} onClick={() => setState({ urgent: !state.urgent })}>Urgent</Button>
                    <Button className={`${state.important ? '!bg-accent' : ''} !px-6 !py-2 flex-1`} onClick={() => setState({ important: !state.important })}>Important</Button>
                </div>
                <Button className="" onClick={editAdventure}>Save</Button>
                <Button className="" onClick={deleteAdventure}>Delete</Button>
